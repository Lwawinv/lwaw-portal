  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom: 6 }}>
        <span style={{ fontSize:13, color:'#4a5568' }}>{checked.size} of {CHECKLIST.length} complete</span>
      ...
      {phases.map(phase => {
        ...
        return (
          <div key={phase} style={{ background:'#fff', border:'1px solid #dce4ed', borderLeft:`3px solid ${phaseColors[phase]||'#2e6da4'}`, borderRadius:8, marginBottom:14, overflow:'hidden' }}>
            ...
            {items.map(item => (
              <div key={item.id} onClick={() => toggle(item.id)} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 16px', cursor:'pointer', borderBottom:'1px solid #f9fafb', background:checked.has(item.id)?'#f0fdf4':'transparent' }}>
                <div style={{ width:18, height:18, borderRadius:3, border:`2px solid ${checked.has(item.id)?(phaseColors[phase]||'#2e6da4'):'#d1d5db'}`, background:checked.has(item.id)?(phaseColors[phase]||'#2e6da4'):'transparent', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  {checked.has(item.id) && <span style={{ color:'#fff', fontSize:11, fontWeight:900 }}>✓</span>}
                </div>
                <span style={{ fontSize:13, color:checked.has(item.id)?'#9ca3af':'#1c2026', textDecoration:checked.has(item.id)?'line-through':'none' }}>{item.text}</span>
              </div>
            ))}
          </div>
        )
