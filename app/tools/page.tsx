...
const s = {
  card: { background: '#fff', border: '1px solid #dce4ed', borderRadius: 10, padding: '22px 26px', marginBottom: 20 },
  label: { fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: '#4a5568', fontWeight: 600, marginBottom: 6, display: 'block' },
  input: { width: '100%', padding: '9px 13px', border: '1px solid #dce4ed', borderRadius: 5, fontSize: 14, fontFamily: "'DM Sans', sans-serif", color: '#1c2026', outline: 'none', boxSizing: 'border-box' },
  ...
  btnBlue: { background: '#2e6da4', color: '#fff', border: 'none', padding: '11px 24px', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  btnGreen: { background: '#15803d', color: '#fff', border: 'none', padding: '11px 24px', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" },
  section: { fontSize: 12, fontWeight: 700, color: '#2e6da4', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10, marginTop: 20, paddingBottom: 6, borderBottom: '1px solid #e8f2fb' },
  output: { background: '#f7f9fc', border: '1px solid #dce4ed', borderRadius: 8, padding: '20px 24px', fontFamily: 'monospace', fontSize: 13, lineHeight: '1.8', whiteSpace: 'pre-wrap', marginTop: 16 },
}
...
function DocOrderForm() {
  const [form, setForm] = useState({ lenderEntity: 'A2DSTX, LLC', hasPrepay: false, taxesEscrowed: false, insuranceEscrowed: false })
...
function ClosingFeesCalc() {
  const [form, setForm] = useState({ txFee: '500', attyFee: '555', attyPages: '13', titleCompany: 'Texas Legacy Land Title' })
  const [output, setOutput] = useState('')
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
  const generate = () => {
    const f = form
...
function NewDealChecklist() {
  ...
  const phaseColors = { 'Pre-Contract':'#2e6da4','Buyer Onboarding':'#15803d','Title & Taxes':'#b45309','Spreadsheet':'#7c3aed','Insurance':'#f97316','Closing Prep':'#dc2626','Documents':'#0891b2','Post-Close':'#15803d' }
  return (
    <div>
